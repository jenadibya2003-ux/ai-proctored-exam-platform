  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${API_BASE}/auth/me`, {
        ...
        .then((data) => {
          if (data) {
            setFullName(data.full_name || "student1");
            setEmail(data.email || "student1@example.com");
            setRole(data.role ? data.role.charAt(0).toUpperCase() + data.role.slice(1) : "Student");
          }
        })
        .catch(() => {});
    }
        ...
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: "16px", padding: "1.6rem" }}>
          ...
          <form onSubmit={handlePasswordUpdate} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.45rem" }}>
              ...
              />
              <label htmlFor="show-passwords" style={{ fontSize: "0.78rem", color: textSub, cursor: "pointer" }}>
                Show passwords
              </label>
            </div>
